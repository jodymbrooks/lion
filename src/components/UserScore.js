...
class UserScore extends Component {
  userInputChange(event) {
    this.props.dispatch(
      updatePlayerName(event.currentTarget.value, this.props.index)
    );
  }
    ...
    return (
      <div className="UserScore" data-isactive={isActive}>
        ...
        <div className="status-section setsArea">
          <span className="statusTitle ui label">Sets: </span>
          <span className="status-count ui basic label">{sets}</span>
        </div>
        <div className="status-section points-area">
          <span className="statusTitle ui label">Points: </span>
          <span className="status-count ui basic label">{points}</span>
        </div>
      </div>
    );
  }
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (isActive !== prevProps.isActive && isActive === true) {
